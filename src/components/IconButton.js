...
export const IconButton = (props) => {
  ...
  const baseStyle =
    'inline-flex w-10 h-10 text-white bg-blue-600 hover:bg-blue-700 p-2.5 focus:outline-none font-medium rounded-full text-center text-sm ml-2';
  const svg = {
    '-delete': (
      <svg
        aria-hidden="true"
  ...
  return (
    <button
      className={`${baseStyle} ${optionalStyle || ''}`}
      onClick={onClick}
      type="button"
    >
      {svg[clazz] || ''}
